...
public class ReportPacienteExamenesDB {
    ...
    private static final String SELECT = "SELECT fecha_solicitado, fecha_finalizada, t.nombre as examen, estado, u.nombre \n"
    ...
    private static final String SELECT_BETWEEN_FECHA = "SELECT fecha_solicitado, fecha_finalizada, t.nombre as examen, estado, u.nombre \n"
            + "FROM solicitud_examen s\n"
            + "INNER JOIN examenes_solicitud e\n"
            + "ON s.id = e.solicitud\n"
            + "INNER JOIN tipo_examen t\n"
    ...
    private static final String SELECT_BY_TIPO_EXAMEN = "SELECT s.id as solicitud fecha_solicitado, fecha_finalizada, t.nombre as examen, estado, u.nombre \n"
            + "FROM solicitud_examen s\n"
            + "INNER JOIN examenes_solicitud e\n"
            + "ON s.id = e.solicitud\n"
            + "INNER JOIN tipo_examen t\n"
    ...
    public List<ReportPacienteExamenes> getList(int idPaciente, String tipoExamen) {
        List<ReportPacienteExamenes> list = new ArrayList<>();
        try (PreparedStatement statement = ConeccionDB.getConnection().prepareStatement(SELECT_BY_TIPO_EXAMEN)) {
            statement.setInt(1, idPaciente);
            statement.setString(2, tipoExamen);
            while (resultSet.next()) {
                list.add(get(resultSet));
            }
        } catch (SQLException ex) {
    ...
    private ReportPacienteExamenes get(ResultSet resultSet) throws SQLException {
        return ReportPacienteExamenes.builder().
                ...
                laboratorio(resultSet.getString("nombre")).build();
    }
}
